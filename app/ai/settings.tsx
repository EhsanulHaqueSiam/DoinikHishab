...
export default function AISettingsScreen() {
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <ScrollView
        className="flex-1"
        contentContainerClassName="px-4 py-5 gap-4 pb-12"
      ...
      >
        <Card>
          ...
          <View className="flex-row flex-wrap gap-2">
            {PROVIDERS.map((p) => (
              ...
              >
                <Text
                  className={`text-xs font-semibold ${provider === p.id ? "text-white" : "text-foreground"}`}
                >
                  {p.name}
                </Text>
              </Pressable>
            ...
            {currentProvider.models.map((m) => (
              <Pressable
                ...
                className={`px-4 py-3 rounded-xl border flex-row items-center justify-between ${
                  model === m
                    ? "bg-surface-300 border-primary-500"
                    : "bg-surface-200 border-border/40"
                }`}
              >
                <Text
                  className={`text-xs ${model === m ? "text-primary-700 font-bold" : "text-foreground font-medium"}`}
                >
                  {m}
                </Text>
                {model === m && (
        ...
        <Card>
          ...
          {testResult && (
            <View
              className={`mt-3 px-3 py-2 rounded-lg ${testResult.success ? "bg-success/10" : "bg-danger/10"}`}
            >
        ...
        <Card>
          ...
          <Text className="text-2xs text-surface-800 leading-4">
            DoinikHishab uses AI to categorize transactions and answer finance questions. You bring
            your own API key (BYOK) — we never share your data with AI providers beyond what's
