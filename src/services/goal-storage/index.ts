...
import type { GoalDataStore, PayDownGoal, SaveUpGoal } from "./types";
...
export function updateSaveUpGoal(id: string, updates: Partial<Omit<SaveUpGoal, "id">>): void {
  const store = getGoals();
...
export function updatePayDownGoal(id: string, updates: Partial<Omit<PayDownGoal, "id">>): void {
  const store = getGoals();
