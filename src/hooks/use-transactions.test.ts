...
const mockTransactions = [
  { _id: "t1", date: "2026-03-15", amount: -5000, type: "expense", description: "Lunch", memo: null, payeeId: null, categoryId: "c1", source: "manual", isCleared: true, isReconciled: false, flag: null, accountId: "a1" },
  { _id: "t2", date: "2026-03-14", amount: 100000, type: "income", description: "Salary", memo: "March salary", payeeId: null, categoryId: "c2", source: "manual", isCleared: false, isReconciled: false, flag: null, accountId: "a1" },
  { _id: "t3", date: "2026-03-13", amount: -2000, type: "expense", description: "Bus fare", memo: null, payeeId: null, categoryId: "c1", source: "manual", isCleared: true, isReconciled: false, flag: "red", accountId: "a1" },
];
...
describe("useFilteredTransactions", () => {
  ...
  it("returns all transactions when no filters applied", () => {
    ...
    const { result } = renderHook(() =>
      useFilteredTransactions("user1" as any)
    );
...
describe("useTransactionSearch", () => {
  ...
  it("returns empty array for empty query", () => {
    mockUseQuery.mockReturnValue(mockTransactions);
    const { result } = renderHook(() =>
      useTransactionSearch("user1" as any, "")
    );
    expect(result.current).toEqual([]);
  });
  ...
  it("searches by description", () => {
    mockUseQuery.mockReturnValue(mockTransactions);
    const { result } = renderHook(() =>
      useTransactionSearch("user1" as any, "lunch")
    );
    expect(result.current).toHaveLength(1);
    expect(result.current[0]._id).toBe("t1");
  });
  ...
  it("searches by memo", () => {
    mockUseQuery.mockReturnValue(mockTransactions);
    const { result } = renderHook(() =>
      useTransactionSearch("user1" as any, "March salary")
    );
    expect(result.current).toHaveLength(1);
