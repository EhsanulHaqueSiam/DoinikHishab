...
export function useImport() {
  ...
  const pickFile = useCallback(async () => {
    ...
    try {
      ...
      // Parse the statement
      const importResult = await parseStatement(
        data,
        isBinary,
        fileName,
        mimeType,
      ...
      const existing =
        getJSON<ParsedTransaction[]>(IMPORT_STORAGE_KEY) ?? [];
  ...
  const importSelected = useCallback(async () => {
    ...
    try {
      ...
      toast.success(
        `Imported ${selectedTransactions.length} transactions`
      );
