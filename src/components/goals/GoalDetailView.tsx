...
import type { GoalStatus, SaveUpGoal } from "../../services/goal-storage/types";
...
export function GoalDetailView({ goal, onEdit, onDelete }: GoalDetailViewProps) {
  ...
  const status = useMemo(
    () =>
      calculateGoalStatus(goal.currentAmount, goal.targetAmount, goal.targetDate, goal.createdDate),
    [goal.currentAmount, goal.targetAmount, goal.targetDate, goal.createdDate]
  );

  const monthlyNeeded = useMemo(
    () => calculateMonthlyContribution(goal.targetAmount, goal.currentAmount, goal.targetDate),
      ...
      <View className="px-4 py-6">
        ...
        <Card className="mb-6">
          <View className="items-center py-4">
            <Text className="text-hero font-bold text-foreground mb-2">{percentage}%</Text>
            ...
            <Text className={`text-2xs font-bold ${config.color}`}>{statusLabel}</Text>
          </View>
        </Card>

        {/* Info rows */}
        <Card className="mb-6">
          <InfoRow label={t("goals.targetDate" as any)} value={targetDateFormatted} />
          <InfoRow
            label={t("goals.monthlyNeeded" as any, { amount: "" })
              .replace("needed", "")
              .trim()}
