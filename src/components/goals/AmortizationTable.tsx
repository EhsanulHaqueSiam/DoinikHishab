...
export function AmortizationTable({ rows, isExpanded }: AmortizationTableProps) {
  ...
  return (
    <Animated.View style={animatedStyle} className="mt-3">
      ...
      {/* Table rows */}
      {visibleRows.map((row) => (
        <View key={row.month} className="flex-row px-2 py-1.5 border-b border-border/15">
          <Text className="flex-1 text-2xs text-surface-800">{row.month}</Text>
      ...
      {/* Show more */}
      {hasMore && !showAll && (
        <Pressable className="py-2 items-center" onPress={() => setShowAll(true)}>
          <Text className="text-xs font-bold text-primary-500">{t("goals.showMore" as any)}</Text>
        </Pressable>
      )}
    </Animated.View>
  );
