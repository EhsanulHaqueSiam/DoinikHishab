...
export function SubscriptionCard({ subscription, onConfirm, onDismiss }: SubscriptionCardProps) {
  ...
  return (
    ...
    >
      {/* Payee name */}
      <Text className="text-sm font-bold text-foreground">{subscription.payee}</Text>
      ...
      <View className="flex-row items-center gap-3 mt-2">
        ...
        <Text className="text-2xs text-surface-800">{subscription.occurrences}x</Text>
      </View>
      ...
      <View className="flex-row gap-3 mt-3">
        ...
        >
          <Text className="text-white font-bold text-sm">{t("recurring.confirm")}</Text>
        </Pressable>
