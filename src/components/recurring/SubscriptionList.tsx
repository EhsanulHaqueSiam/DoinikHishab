    ...
    return (
      ...
      >
        <View
          ...
          accessibilityActions={[{ name: "delete", label: t("recurring.delete") }]}
          onAccessibilityAction={(event) => handleAccessibilityAction(item.id, event)}
        >
          {/* Category icon placeholder */}
          <View className="w-8 h-8 rounded-full bg-surface-300 items-center justify-center">
            <Text className="text-xs font-bold text-surface-900">{initial}</Text>
          </View>

          {/* Center: payee + frequency */}
          <View className="flex-1 ml-3">
            <Text className="text-sm font-bold text-foreground">{item.payee}</Text>
          ...
          {/* Right: amount */}
          <Text className="text-sm font-bold text-surface-900">{formatCurrency(item.amount)}</Text>
        </View>
      </Swipeable>
    );
  };
