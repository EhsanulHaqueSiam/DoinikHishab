...
jest.mock("lucide-react-native", () => {
  ...
  const { View } = require("react-native");
  return {
    Trash2: (props: any) => React.createElement(View, { testID: "trash-icon", ...props }),
  };
});
...
describe("SubscriptionList", () => {
  it("renders subscription payee names and amounts", () => {
    ...
    const { getByText } = render(<SubscriptionList subscriptions={subs} onRemove={jest.fn()} />);
  ...
  it("shows empty state when no subscriptions", () => {
    const { getByText } = render(<SubscriptionList subscriptions={[]} onRemove={jest.fn()} />);

    expect(getByText("No Subscriptions Yet")).toBeTruthy();
    expect(
      getByText("Add subscriptions manually or let us detect them from your transaction history.")
    ).toBeTruthy();
  });
  ...
  it("calls onRemove when accessibility delete action triggered", () => {
    ...
    const { getByText } = render(<SubscriptionList subscriptions={subs} onRemove={onRemove} />);
