...
import { Briefcase, Calendar, RefreshCw, Clock } from "lucide-react-native";
...
export function RuleCard({ rule, width }: RuleCardProps) {
  ...
  return (
    <View
      className="bg-surface-200 rounded-2xl p-6 mx-4"
      style={{ width }}
    >
      ...
      <Text className="text-sm text-surface-900 mt-2 leading-5">
        {t(rule.descKey as any)}
      </Text>

      <View className="mt-4 pl-3 border-l-2" style={{ borderLeftColor: "#e6a444" }}>
        <Text className="text-sm text-surface-800 leading-5">
          {t(rule.exampleKey as any)}
        </Text>
      </View>
    </View>
  );
