...
describe("SankeyDiagram", () => {
  ...
  it("renders without crashing", () => {
    const { toJSON } = render(<SankeyDiagram spending={mockSpending} totalIncome={500000} />);
    expect(toJSON()).toBeTruthy();
  });

  it("renders empty state when no spending data", () => {
    const { getByText } = render(<SankeyDiagram spending={[]} totalIncome={0} />);
    expect(getByText("reports.noCashFlowTitle")).toBeTruthy();
  });

  it("renders cash flow title", () => {
    const { getByText } = render(<SankeyDiagram spending={mockSpending} totalIncome={500000} />);
