...
describe("PrivacyToggle", () => {
  it("renders Eye icon when privacy is off", () => {
    const { getByTestId } = render(<PrivacyToggle isPrivacy={false} onToggle={jest.fn()} />);
    expect(getByTestId("eye-icon")).toBeTruthy();
  });

  it("renders EyeOff icon when privacy is on", () => {
    const { getByTestId } = render(<PrivacyToggle isPrivacy={true} onToggle={jest.fn()} />);
    expect(getByTestId("eye-off-icon")).toBeTruthy();
  });

  it("calls onToggle when pressed", () => {
    const onToggle = jest.fn();
    const { getByRole } = render(<PrivacyToggle isPrivacy={false} onToggle={onToggle} />);
    fireEvent.press(getByRole("switch"));
    expect(onToggle).toHaveBeenCalledTimes(1);
  });

  it("has correct accessibility attributes", () => {
    const { getByRole } = render(<PrivacyToggle isPrivacy={true} onToggle={jest.fn()} />);
