...
export function AoMTrendChart({ data }: AoMTrendChartProps) {
  ...
  const valueColor = currentAoM != null && currentAoM >= 30 ? "text-primary" : "text-foreground";

  // Trend indicator
  const trendArrow = trend === "improving" ? " ^" : trend === "declining" ? " v" : "";
  const trendColor =
    trend === "improving"
      ? "text-green-500"
      : trend === "declining"
      ...
      <View className="flex-row items-baseline justify-between">
        <View>
          ...
          <View className="flex-row items-baseline gap-1">
            ...
            <Text className="text-sm text-muted-foreground">{t("reports.days")}</Text>
          </View>
        </View>
        {trend && (
