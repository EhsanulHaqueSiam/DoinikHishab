...
describe("IncomeExpenseBarChart", () => {
  ...
  it("renders without crashing", () => {
    const { toJSON } = render(<IncomeExpenseBarChart data={mockData} />);
    expect(toJSON()).toBeTruthy();
  });

  it("displays income and expense labels", () => {
    const { getByText } = render(<IncomeExpenseBarChart data={mockData} />);
  ...
  it("renders empty state when no data", () => {
    const { getByText } = render(<IncomeExpenseBarChart data={[]} />);
