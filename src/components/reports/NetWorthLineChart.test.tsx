...
describe("NetWorthLineChart", () => {
  ...
  it("renders without crashing", () => {
    const { toJSON } = render(
      <NetWorthLineChart data={mockData} />,
    );
  ...
  it("shows downward trend when net worth declines", () => {
    const decliningData = [
      {
        month: "2026-01",
        income: 0, expense: 0, net: 0, spendingByCategory: [],
        netWorth: 1000000, ageOfMoney: null, daysOfBuffering: null,
      },
      {
        month: "2026-02",
        income: 0, expense: 0, net: 0, spendingByCategory: [],
        netWorth: 800000, ageOfMoney: null, daysOfBuffering: null,
      },
    ];
    const { getByText } = render(
      <NetWorthLineChart data={decliningData} />,
    );
