...
export function TypeMappingPill({ type, onToggle }: TypeMappingPillProps) {
  ...
  const label = isExpense ? t("transaction.expense") : t("transaction.income");
  ...
  return (
    <Pressable
      ...
      className={`rounded-full px-3 py-1 ${isExpense ? "bg-danger/20" : "bg-primary-500/20"}`}
    >
      <Text className={`text-2xs font-bold ${isExpense ? "text-danger" : "text-primary-700"}`}>
        {label}
      </Text>
    </Pressable>
  );
