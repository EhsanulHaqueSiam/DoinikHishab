...
export function ImportReviewRow({
  ...
  return (
    ...
    >
      ...
      >
        <CheckIcon size={22} color={isSelected ? "#0d9488" : "#6b7280"} />
      </View>

      {/* Transaction info */}
      <View className="flex-1 gap-1">
        <View className="flex-row items-center gap-2">
          <Text className="text-2xs text-surface-800">{formatDate(transaction.date)}</Text>
        ...
        <View className="flex-row items-center gap-2">
          <Text className="text-2xs text-surface-800">Ref: {transaction.reference}</Text>
          {isDuplicate && (
      ...
      <View className="items-end gap-1 ml-2">
        ...
        <TypeMappingPill type={transaction.type} onToggle={onToggleType} />
      </View>
    </Pressable>
  );
}
